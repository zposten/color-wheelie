...
export class TintedPalette {
  constructor(colorWheel, container) {
    this.classNames = {
  ...
  init(colorWheel) {
    ...
    colorWheel.dispatch.on('bindData.themeBuild', data =>
      this.createDOM(colorWheel, data),
    )
    colorWheel.dispatch.on('markersUpdated.theme', () =>
      this.handleMarkersUpdated(colorWheel),
    )
  }

  createDOM(colorWheel, data) {
  ...
  handleMarkersUpdated(colorWheel) {
    this.$container
      .selectAll('.' + this.classNames.paletteColor)
      .each(function (d, i) {
        let swatchEl = this
    ...
    this.$container.selectAll('.' + this.classNames.swatch).each(function (d) {
      let swatchEl = this
      swatchEl.style.backgroundColor = chroma(d.color).hex()
    })

    this.$container.selectAll('.' + this.classNames.slider).each(function (d) {
      let sliderEl = this
